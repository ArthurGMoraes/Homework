...
// world->set ( 4, 4, VWALL );
// world->set ( 4, 4, HWALL );
BHWall(2,7,8);BHWall(2,7,2);BHWall(3,7,7);BHWall(5,6,4);world->set (6,6, HWALL );world->set (4,6, HWALL );
...
for (int Q=2;Q>0;Q--){world->set ( 5, 7, BEEPER );}
for (int Q=3;Q>0;Q--){world->set ( 6, 7, BEEPER );}
...
void turnAroundCornerLeft( ){move();turnLeft();move();turnLeft();move();}
...
{
	pickBeeper();
	std::ofstream arquivo("Mapa0313.txt", std::ios_base::app);
int x, y;
...
void fazerCoisa ()
{ clearFile("Mapa0313.txt");execute(5);execute(8);execute(5);execute(6);
followLeftWall(95);execute(55);execute(6);} 
...
set_Speed ( 0.2 ); // definir velocidade padrao
// criar robo
