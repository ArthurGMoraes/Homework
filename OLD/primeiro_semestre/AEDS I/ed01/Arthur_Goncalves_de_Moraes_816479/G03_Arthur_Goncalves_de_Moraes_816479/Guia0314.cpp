...
// world->set ( 4, 4, HWALL );
BRectangle(5,5,5,5);
BHWall(2,7,8);BHWall(2,7,2);BHWall(3,7,7);world->set (6,6, HWALL );world->set (4,6, HWALL );world->set (3,3, HWALL );
BVWall(1,3,8);BVWall(2,4,7);BVWall(7,3,7);BVWall(3,5,6);BVWall(6,5,6);world->set (4,7, VWALL );world->set (5,7, VWALL );
// colocar um marcador no mundo
world->set ( 5, 6, BEEPER );
for (int Q=2;Q>0;Q--){world->set ( 6, 7, BEEPER );}
for (int Q=3;Q>0;Q--){world->set ( 4, 7, BEEPER );}
...
// comando invalido
show_Error ( "ERROR: Invalid value." );
} // end switch
} // end execute( )
...
if (  
		(0 <= action && action <= 9)
		|| (action == 77) || (action == 99) || (action == 55) || (action == 111) || (action == 11) 
	)
{
// executar comando
execute ( action );
...
{bool Res = frontIsClear();while(Res){movePP();Res = frontIsClear();}}
...
void turnAroundCornerLeft( ){movePP();turnLeft();movePP();turnLeft();movePP();}
...
void followRightWall(int X)//X= vezes que vai fazer
{
	while(X>0)
	{
		X--;
...
{
	move();
	std::ofstream arquivo("Mapa0314.txt", std::ios_base::app);
int x, y;
...
void clearFile (const char* Pedro){std::ofstream ofs;
ofs.open(Pedro, std::ofstream::out | std::ofstream::trunc);
ofs << world->avenues( ) << "\n";
ofs << world->streets( ) << "\n";
ofs.close();}

void fazerCoisa ()
{ clearFile("Mapa0314.txt");execute(5);execute(8);execute(5);execute(6);
followLeftWall(25);followRightWall(58);
execute(55);execute(4);execute(55);execute(6);readMap("Mapa0314.txt");} 
...
---------------------------------------------- previsao de testes
v14.0.0
	vou ver o minimo possivel de passos necessarios para chegar á uma posição satisfatória em que o followLeftWall só seje usado uma vez
---------------------------------------------- historico
v14.0.0
	
---------------------------------------------- testes
Versao Teste
0.1 0.0 ( OK ) identificacao de programa
0.2     ( OK )
0.3     ( OK )
0.4 0.1 ( OK )
0.5     ( OK )
...
0.9     ( OK )
1.0     ( OK )
1.1 0.0 ( OK ) Exercicio 1
1.2 0.0 ( OK ) Exercicio 2
1.3 0.0 ( OK ) Exercicio 3
1.4 0.0 ( OK ) Exercicio 4
*/